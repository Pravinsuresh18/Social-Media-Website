...
function Home() {
  ...
  const profileFun =async()=>{
     ...
     await fetch('http://localhost:6222/getprofiledb', requestOptions)
     .then(response => response.json())
     .then((params) => {
       setprofile(params.sucess)
  ...
  const getpostfun=async()=>{
    setmob(value.state)
     ...
     await fetch('http://localhost:6222/getpostdb', requestOptions)
     .then(response => response.json())
     .then((params) => {
       setuserpost(params.sucess)
 
    })

  }
  const gettotalfriendsfun=async()=>{
    const requestOptions = {
      method: 'POST',
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({mobile:value.state}),
 ...
 .then((params) => {
  ...
  const commentfun=async()=>{
    if(postcomment.length >= 3){
      const requestOptions = {
        method: 'POST',
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({mobile:value.state,postpic:commentpic,comment:postcomment}),
        redirect: 'follow'
      };
    
      await fetch('http://localhost:6222/insertcommentdb', requestOptions)
      .then(response => response.json())
      .then((params) => {
        alert(params.sucess)
        friendslist()
  ...
  const likefun=async(postpic,usermobile)=>{
    ...
    const requestOptions = {
      method: 'POST',
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({mobile:value.state,postpic:postpic,umobile:value.state}),
      redirect: 'follow'
    };
...
const logout=()=>{
  ...
  <nav id="sidebarMenu" style={{width:'17%',height:'100%',minHeight:'650px',position:'relative',marginTop:'77px'}} className="col-lg-2 d-none   d-lg-block d-xl-block  sidebar navbar-dark gradient-sidebar  ">
      ...
      <div className="list-group list-group-flush mx-3 gap-2  ">
        ...
        <button className="d-flex list-group-item list-group-item-action py-2 ripple" onClick={()=>{navigate("/Post",{state:mob})}}><i
        ...
        <button className="d-flex list-group-item list-group-item-action py-2 ripple" onClick={() => {logout()}}><i
  ...
  <div style={{width:'55%',height:'100vh',marginLeft:'25%',marginTop:'70px',minHeight:'500px',minWidth:'500px'}} className="col-lg-8 col-12 col-md-8 bodypost ">
    ...
    <div style={{height: "295px", width: "100%",marginLeft:'5%',marginTop:'20px',backgroundImage:`url(${profile[0]?.coverpic})`,backgroundSize: "cover", border:'3px solid #B3B3B3', backgroundRepeat: "no-repeat",borderRadius:'10px'}} className="shadow">
        <div style={{height:'380px',width:'750px' ,justifyContent:'center',marginLeft:'35%'}}>
         <img style={{height:'200px',width:'200px',borderRadius:'90px', border:'3px solid #fff',marginTop:'180px',}} src={profile[0]?.profilepic}></img>
         
         </div>
         <div style={{justifyContent:'center',display:'flex'}}> 
         <div style={{justifyContent:'center',alignItems:'center',}}>       
            <h2 style={{justifyContent:'center',display:'flex'}}>{profile[0]?.username}</h2>
            <h6 style={{fontFamily:'cursive'}}>{profile[0]?.bio}</h6>
            </div> 
        </div>
        <div style={{display:'flex',height:'80%',width:'100%',justifyContent:'center',alignItems:'center',alignContent:'center',gap:'50px',marginTop:'10px'}}>
            <div style={{height:'80%',width:'40%',borderRadius:'10px',backgroundColor:'#D4D4D4',alignItems:'center',display:'flex',justifyContent:'center'}} className="shadow1">
                <div style={{justifyContent:'center',alignItems:'center',alignContent:'center'}}>
                    <h1 style={{fontFamily:'serif',fontSize:'180%',display:'flex',justifyContent:'center'}}>About</h1>
                    <div style={{display:'flex',alignItems:'center',gap:'5px'}}>
                        <h5 style={{fontFamily:'serif',fontSize:'130%'}}> Dob :</h5>
                        <h6 style={{fontFamily:'cursive'}}>{profile[0]?.birthdate}</h6>
                    </div>
                    <div style={{display:'flex',alignItems:'center',gap:'5px'}}>
                        <h5 style={{fontFamily:'serif'}}> Gender :</h5>
                        <h6 style={{fontFamily:'cursive'}}>{profile[0]?.gender}</h6>
                    </div>
                </div>
            ...
            <div style={{height:'80%',width:'40%',borderRadius:'10px',backgroundColor:'#D4D4D4',alignItems:'center',display:'flex',justifyContent:'center'}} className="shadow1">
                <div style={{justifyContent:'center',alignItems:'center',alignContent:'center'}}>
                    <h1 style={{fontFamily:'serif',fontSize:'180%',display:'flex',justifyContent:'center'}}>Friends</h1>
                    <div style={{display:'flex',alignItems:'center',gap:'5px'}}>
                        <h5 style={{fontFamily:'serif'}}> Total Friends :</h5>
                        <h6 style={{fontFamily:'cursive'}}>{totalfriends.length}</h6>
                    </div>
        ...
        <span style={{width:'750px',marginTop:'10px',marginBottom:'50px',minHeight:'250px',minWidth:'250px',background:'#D4D4D4',borderRadius:'10px',}} className="shadow2" >
          <div style={{display:'flex',justifyContent:'center'}}>
            <h3 style={{fontFamily:'serif',marginTop:'3%'}}>Post</h3>
          </div>
          { userpost?.map((element,index)=>{
            return(
              <div style={{}}>
              <div style={{display:'flex',gap:'15px'}}>
              <img style={{height:'50px',width:'50px',borderRadius:'25px',marginLeft:'100px'}} src={profile[0]?.profilepic}></img>
              <h5 style={{marginTop:'19px'}}>{profile[0]?.username}</h5>
              </div>
            <div style={{display:'flex',justifyContent:'center'}}>
              <span style={{width:'500px',background:'#ffffff',borderRadius:'10px',justifyContent:'center',marginTop:'10px',marginBottom:'30px'}}>
                <span style={{margin:'20px',fontFamily:'cursive',fontSize:'15px',marginTop:'10px'}}>{element?.postbio}</span>
              <img style={{height:'430px',width:'480px',borderRadius:'10px',margin:'10px'}} className="picshadow" src={element?.postpic}></img>
              <div style={{display:'flex' ,marginLeft:'50px',gap:'20px',marginTop:'5px'}}>
                
                   {/* {element?.like?.mobile[index] ===  ?
                   <span onClick={()=>setColor()} style={{border:'0px solid'}}><i style={{fontSize:'30px'}} className=" fa-regular fa-thumbs-up fa-shake"></i></span>:
               ...
               */}
                ...
                <button  style={{border:'0px solid',backgroundColor:'#ffffff'}} onClick={()=>{likefun(element.postpic)}}><i style={{fontSize:'30px'}} className=" fa-regular fa-thumbs-up fa-shake"></i></button>
              {/* {userpost[0]?.likes[0]?.map((ele,ind)=>{
                ele.mobile=='181818181' ?                                                 
                  <span onClick={()=>{setlike(true)}} style={{border:'0px solid'}}><i style={{fontSize:'30px'}} className="text-danger fa-regular fa-thumbs-up fa-shake"></i></span>:<span onClick={()=>setColor()} style={{border:'0px solid'}}><i style={{fontSize:'30px'}} className=" fa-regular fa-thumbs-up fa-shake"></i></span>
                ...
                <div style={{display:'flex',gap:'5px',marginTop:'5px'}}>
                  <h5 style={{fontFamily:'cursive'}}>{userpost[index]?.likes.length}</h5> 
                <h5  style={{fontFamily:'cursive'}}>liked</h5>
                </div>
                <button  type="button" data-bs-toggle="modal" data-bs-target="#myModal" style={{border:'0px solid',marginLeft:'110px',backgroundColor:'#ffffff'}} onClick={()=>{setcommentpic(element.postpic)}}> <i style={{fontSize:'30px'}} className="fa-sharp fa-regular fa-comment fa-fade"></i></button>
                <div style={{display:'flex',gap:'5px',marginTop:'5px'}}>
                        <h5 style={{fontFamily:'cursive'}}> {userpost[index]?.comments.length}</h5> 
              ...
              <span>
                ...
                {userpost[index].comments?.map((ele,ind)=>{
                  return(
                    <span>
